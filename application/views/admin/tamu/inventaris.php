                ...
                <a href="<?= base_url('mainpage') ?>" class="btn btn-danger mt-3 mb-3"><i class="fa-solid fa-arrow-left"></i> Kembali ke dahsboard</a>
                    ...
                    <table class="table text-center table-striped">
                        ...
                        <tbody>
                            ...
                            $i = 1;
                            if(!$result){
                                echo "<p class='alert alert-danger text-center mt-5 mb-5'>Tidak ada data inventaris</p>";
                            }else{    
                            foreach ($result as $invent){
                            ?>
                            <tr>
                                
                                <td><?= $invent->id ?></td>
                                <td><?= $invent->id_transaksi ?></td>
                                <td><?= $invent->cek_in  ?></td>
                                <td><?= $invent->cek_out  ?></td>
                                <td><?= $invent->jumlah  ?></td>
                                <td><?= $invent->jenis  ?></td>
                                <td><?= $invent->firstname.' '. $invent->lastname  ?></td>
                                <td><?= $invent->email  ?></td>
                                <td><?= $invent->notelp ?></td>         
                            </tr>
                            <?php  } ?>
